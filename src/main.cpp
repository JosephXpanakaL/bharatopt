...
int main(int argc,char**argv){
 bool demo=false,mip_demo=false,qp_demo=false,ip=false,use_cuda=false,json=false,lp_only=false;std::string mps;bharatopt::SolverOptions opt;
 for(int i=1;i<argc;i++){std::string a=argv[i];if(a=="--demo")demo=true;else if(a=="--mip-demo")mip_demo=true;else if(a=="--qp-demo")qp_demo=true;else if(a=="--ip")ip=true;else if(a=="--cuda")use_cuda=true;else if(a=="--json")json=true;else if(a=="--lp-only")lp_only=true;else if(a=="--mps"&&i+1<argc)mps=argv[++i];else if(a=="--max-iters"&&i+1<argc)opt.max_iterations=std::stoi(argv[++i]);else if(a=="--tol"&&i+1<argc)opt.tolerance=std::stod(argv[++i]);else if(a=="--max-nodes"&&i+1<argc)opt.max_nodes=std::stoi(argv[++i]);else if(a=="--mip-gap"&&i+1<argc)opt.mip_gap=std::stod(argv[++i]);else if(a=="--time-limit"&&i+1<argc)opt.time_limit_sec=std::stod(argv[++i]);else if(a=="--no-presolve")opt.presolve=false;else if(a=="--scaling-passes"&&i+1<argc)opt.scaling_passes=std::stoi(argv[++i]);else{std::cerr<<"Unknown argument: "<<a<<"\n";return 2;}}
 try{
  ...
  bharatopt::LPModel m;if(demo)m=bharatopt::make_refinery_demo();else if(mip_demo)m=bharatopt::make_milp_demo();else if(!mps.empty())m=bharatopt::parse_mps(mps);else{std::cerr<<"Use --demo, --mip-demo, --qp-demo, --ip or --mps file.mps\n";return 2;}
  auto r=ip?bharatopt::solve_interior_point(m, bharatopt::InteriorPointOptions{opt.max_iterations,opt.tolerance,400,400}):(lp_only?solver.solve_lp(m,opt):solver.solve(m,opt));
