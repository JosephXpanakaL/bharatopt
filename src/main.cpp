...
int main(int argc,char**argv){
 bool demo=false,mip_demo=false,use_cuda=false,json=false,lp_only=false; std::string mps;
 bharatopt::SolverOptions o;
 for(int i=1;i<argc;i++){std::string a=argv[i];
  if(a=="--demo")demo=true;else if(a=="--mip-demo")mip_demo=true;else if(a=="--cuda")use_cuda=true;else if(a=="--json")json=true;else if(a=="--lp-only")lp_only=true;
 ...
 try{
  bharatopt::LPModel m;if(demo)m=bharatopt::make_refinery_demo();else if(mip_demo)m=bharatopt::make_milp_demo();else if(!mps.empty())m=bharatopt::parse_mps(mps);else{std::cerr<<"Use --demo, --mip-demo or --mps file.mps\n";return 2;}
