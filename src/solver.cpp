...
void certify_original(const LPModel&orig,SolverResult&r){std::vector<double>ax;Ax(orig.A,r.x,ax);double viol=0;for(std::size_t i=0;i<orig.A.rows;i++){double w=0; if(ax[i]<orig.row_lower[i])w=orig.row_lower[i]-ax[i];else if(ax[i]>orig.row_upper[i])w=ax[i]-orig.row_upper[i];viol+=w*w;}r.primal_residual=std::sqrt(viol)/(1.0+n2(ax));double internal=0;for(std::size_t j=0;j<r.x.size();j++)internal+=orig.objective[j]*r.x[j];r.objective=(orig.maximize?-internal:internal)+orig.objective_offset;r.best_bound=r.objective;}
