...
bool integer_feasible(const LPModel&m,const std::vector<double>&x,double tol,int&branch){
  branch=-1;
  for(size_t i=0;i<m.integer.size();++i) if(m.integer[i]){
    double nearest=std::round(x[i]); if(std::abs(x[i]-nearest)>tol){branch=(int)i;return false;}
  }
  return true;
}
}
SolverResult BharatOptSolverCore::solve_lp(const LPModel&m,const SolverOptions&o){
  if(m.A.cols!=m.objective.size()||m.A.rows!=m.row_lower.size()||m.A.rows!=m.row_upper.size()||
     m.lower.size()!=m.A.cols||m.upper.size()!=m.A.cols) throw std::runtime_error("Inconsistent model dimensions");
  int n=(int)m.A.cols, rc=(int)m.A.rows; SolverResult r;r.x.assign(n,0);proj(r.x,m.lower,m.upper);
  std::vector<double>xbar=r.x,y(rc),a,aty;double L=opnorm(m.A);if(L<1e-12)L=1;double tau=o.tau,sigma=o.sigma;
  ...
  for(int it=1;it<=o.max_iterations;it++){
    ...
    for(int j=0;j<n;j++)xbar[j]=r.x[j]+o.theta*(r.x[j]-xbar[j]);
    ...
    double ds=0;for(int j=0;j<n;j++){double z=r.x[j]-(m.objective[j]+aty[j]);double p=std::min(std::max(z,m.lower[j]),m.upper[j]);double d=r.x[j]-p;ds+=d*d;}r.dual_residual=std::sqrt(ds)/(1+n2(m.objective));
    r.iterations=it;
    if(o.verbose&&(it==1||it%1000==0)){}
    if(std::max(r.primal_residual,r.dual_residual)<=o.tolerance){r.converged=true;break;}
  }
  for(int j=0;j<n;j++)r.objective+=m.objective[j]*r.x[j];
  r.best_bound=r.objective;r.mip_gap=0;
  r.solve_time_sec=std::chrono::duration<double>(std::chrono::steady_clock::now()-t0).count();
  r.status=r.converged?"OPTIMALITY_TOL_REACHED":"ITERATION_LIMIT";return r;
}
SolverResult BharatOptSolverCore::solve_milp(const LPModel&m,const SolverOptions&o){
  auto t0=std::chrono::steady_clock::now(); SolverResult out;out.backend="CPU-branch-and-bound";
  out.x.assign(m.A.cols,0);double incumbent=INF;double best_bound=INF;size_t nodes=0;bool hit_limit=false;
  std::function<void(const LPModel&,int)> dfs = [&](const LPModel&node,int depth){
    ...
    SolverResult rr=solve_lp(node,lp);
    if(!finite_solution(rr)||!rr.converged)return;
    best_bound=std::min(best_bound,rr.objective);
    if(rr.objective>=incumbent-1e-9)return;
    int branch=-1;
    if(integer_feasible(node,rr.x,o.integrality_tolerance,branch)){
      if(rr.objective<incumbent){incumbent=rr.objective;out.x=rr.x;}
      return;
    }
    if(branch<0)return;
    double value=rr.x[branch];double fl=std::floor(value), ce=std::ceil(value);
    if(fl>=node.lower[branch]){
      LPModel left=node;left.upper[branch]=std::min(left.upper[branch],fl);
      if(left.lower[branch]<=left.upper[branch])dfs(left,depth+1);
    }
    if(ce<=node.upper[branch]){
      LPModel right=node;right.lower[branch]=std::max(right.lower[branch],ce);
      if(right.lower[branch]<=right.upper[branch])dfs(right,depth+1);
    }
  };
  dfs(m,0);out.nodes=nodes;out.iterations=(int)nodes;out.best_bound=best_bound;out.objective=incumbent;
  if(std::isfinite(incumbent)){
    double denom=1+std::abs(incumbent);out.mip_gap=std::isfinite(best_bound)?std::max(0.0,(incumbent-best_bound)/denom):INF;
    out.converged=!hit_limit && out.mip_gap<=o.mip_gap;
    out.status=out.converged?"MIP_OPTIMALITY_GAP_REACHED":(hit_limit?"MIP_NODE_LIMIT":"MIP_INCUMBENT_FOUND");
  }else{
    out.status=hit_limit?"MIP_NODE_LIMIT_NO_INCUMBENT":"MIP_NO_FEASIBLE_INCUMBENT";
  }
